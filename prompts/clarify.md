...
**Output language:** ENGLISH in `` fields (legacy naming; content decoupled).
...
- `question` formatted strictly as `"<short direct English question> (— used to <one-line purpose>)"`. If a purpose clause exists, refine it; if missing, infer it from the field name and the object/problem context.
...
- Field name `question` stores English content. Same legacy-naming convention as the rest of the project.
